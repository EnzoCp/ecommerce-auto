...
def cart_detail(request, total=0, counter=0, cart_items=None):
    ...
    description = 'Pascucci Development - New Order'
    ...
    if request.method == "POST":
        try:
            ...
            shippingCity = request.POST['stripeBillingAddressCity']
    ...
    return render(request, 'store/cart.html', dict(cart_items=cart_items,
...
def orderHistory(request):
    if request.user.is_authenticated:
        ...
        order_detail = Order.objects.filter(emailAddress=email)
        return render(request, 'store/order_list.html', {'order_detail': order_detail})
...
def viewOrder(request, order_id):
    ...
    return render(request, 'store/order_detail.html', {'order': order, 'order_item': order_items})
